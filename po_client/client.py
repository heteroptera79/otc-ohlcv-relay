...
# ── Dead-region rotation (module-level, shared across instances) ──────────
# When a region fails to connect we mark it dead for TTL seconds. Next
# _start_regular_connection() filters those out. When TTL expires the region
# quietly returns to the pool. If ALL regions end up dead we fall back to
# trying everything anyway rather than hanging with no candidates.
_DEAD_REGIONS: dict[str, float] = {}


def _dead_ttl_s() -> float:
    try:
        return float(os.environ.get("PO_DEAD_REGION_TTL_S", "300"))
    except ValueError:
        return 300.0


def _mark_region_dead(name: str) -> None:
    _DEAD_REGIONS[name.upper()] = time.monotonic() + _dead_ttl_s()
    logger.info(f"region {name}: marked dead for {_dead_ttl_s():.0f}s")


def _mark_region_alive(name: str) -> None:
    if _DEAD_REGIONS.pop(name.upper(), None) is not None:
        logger.info(f"region {name}: revived, back in pool")


def _prune_dead_regions() -> set[str]:
    """Drop expired entries, return still-dead region names (uppercase)."""
    now = time.monotonic()
    expired = [n for n, exp in _DEAD_REGIONS.items() if exp <= now]
    for n in expired:
        _DEAD_REGIONS.pop(n, None)
        logger.info(f"region {n}: cooldown expired, back in pool")
    return set(_DEAD_REGIONS.keys())
...
class AsyncPocketOptionClient:
    ...
    async def _start_regular_connection(self, regions: Optional[List[str]] = None) -> bool:
        ...
        if not regions:
            if self.is_demo:
                ...
                # For live mode: full non-demo pool, minus static PO_SKIP_REGIONS,
                # minus dynamically-dead regions (auto-recover after TTL).
                skip = {r.strip().upper() for r in os.environ.get("PO_SKIP_REGIONS", "").split(",") if r.strip()}
                dead = _prune_dead_regions()
                all_regions = REGIONS.get_all_regions()
                base_pool = [name for name, _url in all_regions.items()
                             if "DEMO" not in name.upper() and name.upper() not in skip]
                regions = [n for n in base_pool if n.upper() not in dead]
                # Safety net: if everything is currently marked dead we still try.
                # Better a noisy retry than a silent zero-candidate loop.
                if not regions and base_pool:
                    logger.warning(
                        f"Live mode: all {len(base_pool)} regions currently in dead-set "
                        f"({sorted(dead)}); ignoring dead-set for this attempt"
                    )
                    regions = base_pool
                extras = []
                if skip: extras.append(f"skip={sorted(skip)}")
                if dead: extras.append(f"dead={sorted(dead)}")
                tag = f" ({', '.join(extras)})" if extras else ""
                logger.info(f"Live mode: Using non-demo regions{tag}: {regions}")
        ...
        for region in regions:
            try:
                ...
                if success:
                    ...
                    # connect() returned False without raising — treat as dead
                    _mark_region_dead(region)
    ...
    async def _request_candles(self, asset: str, timeframe: int, count: int, end_time: datetime):
        """Request candle data via loadHistoryPeriod (mastaaa1987-style: live OTC quotes for all assets)."""

        _OFFSET_TABLE = {
            5: 1000, 10: 2000, 15: 3000, 30: 6000, 60: 9000, 120: 18000,
            180: 27000, 300: 45000, 600: 90000, 900: 135000, 1800: 270000,
            3600: 540000, 14400: 2160000, 86400: 12960000,
        }
        offset = _OFFSET_TABLE.get(timeframe, max(timeframe * 150, 9000))
        end_unix = int(end_time.timestamp()) + 7200

        data = {
            "asset": str(asset),
            "index": random.randint(5000, 9999),
            "time": end_unix,
            "offset": offset,
            "period": timeframe,
        }
        ...
        # 2026-07-31 fix: pending queue per (asset, timeframe) instead of a
        # single future — recorder+chipa-fresh often request the same pair
        # concurrently and the previous single-slot map lost the earlier
        # request, causing bogus 10s timeouts and stale-DEFER cascades.
        if not hasattr(self, "_candle_requests"):
            self._candle_requests = {}
        pending = self._candle_requests.setdefault(request_id, [])
        pending.append(candle_future)
        ...
        try:
            # Wait for the response (bumped 10s→20s to survive PO WS hiccups
            # under load; the resolver's own outer timeout still bounds us).
            candles = await asyncio.wait_for(candle_future, timeout=20.0)
            return candles
            ...
            # Remove our future from the pending list (leave others intact).
            lst = self._candle_requests.get(request_id)
            if lst is not None:
                try:
                    lst.remove(candle_future)
                except ValueError:
                    pass
                if not lst:
    ...
    def _parse_candles_data(self, candles_data: List[Any], asset: str, timeframe: int):
        """Parse candles data from server response (supports legacy list & new dict shape)."""
        candles = []
        ...
        try:
            ...
            for candle_data in candles_data:
                if isinstance(candle_data, dict):
                    # New PO format: {'symbol_id':, 'time':, 'open':, 'close':, 'high':, 'low':, 'volume':}
                    if "time" not in candle_data or "close" not in candle_data:
                        continue
                    candle = Candle(
                        timestamp=datetime.fromtimestamp(int(candle_data["time"])),
                    ...
                    # Legacy format: [timestamp, open, close, high, low, volume]
                    candle = Candle(
    ...
    async def _on_json_data(self, data: Dict[str, Any]) -> None:
        ...
        if not isinstance(data, dict):
            return
        # Check if this is candles data response (new "data" key OR legacy "candles" key)
        candles_field = None
        if "data" in data and isinstance(data["data"], list) and data["data"]:
            first = data["data"][0]
            if isinstance(first, dict) and "time" in first and "close" in first:
                candles_field = data["data"]
            elif isinstance(first, (list, tuple)) and len(first) >= 5:
                candles_field = data["data"]
        elif "candles" in data and isinstance(data["candles"], list):
            candles_field = data["candles"]
        if candles_field is not None:
            asset = data.get("asset")
            period = data.get("period")
            if asset and period and hasattr(self, "_candle_requests"):
                request_id = f"{asset}_{period}"
                # 2026-07-31 fix: fanout to ALL pending futures for this
                # (asset, timeframe) — previously only one was set (single-
                # slot map) and concurrent requesters got stuck at timeout.
                pending = self._candle_requests.get(request_id)
                if pending:
                    candles = self._parse_candles_data(candles_field, asset, period)
                    resolved = 0
                    ...
                    if self.enable_logging:
                        logger.success(
                            ...
                            + (f" (fanout={resolved})" if resolved > 1 else "")
                        )
                    return
            return
    ...
    async def _on_candles_received(self, data: Dict[str, Any]) -> None:
        """Handle loadHistoryPeriod placeholder event — actual candles arrive
        in the follow-up binary JSON message handled by _on_json_data. Only
        resolve the future here if the placeholder somehow already carries
        candles; otherwise do nothing and let _on_json_data set the result.
        """
        candles_field = data.get("data") if isinstance(data, dict) else None
        if not (isinstance(candles_field, list) and candles_field):
            candles_field = data.get("candles") if isinstance(data, dict) else None
        if not (isinstance(candles_field, list) and candles_field):
            return
        if hasattr(self, "_candle_requests") and self._candle_requests:
            # 2026-07-31: _candle_requests now maps request_id → list[Future]
            # (was single Future). Adapt iteration accordingly.
            try:
                for request_id, futures in list(self._candle_requests.items()):
                    ...
                    try:
                        ...
                        continue
                    candles = self._parse_candles_data(candles_field, asset, timeframe)
                    if not candles:
                        continue
    ...
    async def _handle_candles_stream(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if hasattr(self, "_candle_requests") and request_id in self._candle_requests:
                # 2026-07-31: list-of-futures format.
                futures = self._candle_requests.get(request_id) or []
