...
class AsyncWebSocketClient:
    ...
    def __init__(self):
        ...
        # 2026-07-29 race-condition fix: держим ссылку на receiver task чтобы
        # при reconnect отменить предыдущий, а не порождать конкурента на recv().
        ...
        # PocketOption stopped sending explicit '42["successauth"]' — first
        # updateAssets / payout bytes frame implicitly confirms auth. One-shot
        # flag so we only emit the synthetic 'authenticated' event once per
        # connection (reset in connect()).
        self._auth_emitted_via_data: bool = False
    ...
    async def connect(self, urls: List[str], ssid: str) -> bool:
        ...
        for url in urls:
            try:
                ...
                # Connect with per-region timeout. Short enough that the outer
                # fetch timeout (8s in chipa-fresh) still has budget to failover
                # to the next region if this one hangs. Env-tunable.
                _ws_timeout = float(os.environ.get("PO_WS_CONNECT_TIMEOUT_S", "4.0"))
                ...
                self._running = True
                self._auth_emitted_via_data = False
    ...
    async def _handle_payout_message(self, json_message: List[List[Any]]) -> None:
        ...
        try:
            ...
            # PO stopped sending explicit successauth — payout frames are our
            # implicit auth confirmation. Emit once per connection.
            if not self._auth_emitted_via_data:
                self._auth_emitted_via_data = True
                await self._emit_event("authenticated", {"source": "payout_frame"})
    ...
    async def _start_background_tasks(self) -> None:
        ...
        # Отменяем предыдущий receiver если он остался жив после reconnect —
        # иначе два корутина одновременно зовут websocket.recv() → RuntimeError
        ...
        # Start message receiving task (store reference — needed for cancel above)
    ...
    async def _handle_json_message(self, data: List[Any]) -> None:
        ...
        # updateAssets is the first server-initiated event after a successful
        # auth handshake since PO removed the explicit successauth frame.
        elif event_type == "updateAssets" and not self._auth_emitted_via_data:
            self._auth_emitted_via_data = True
            await self._emit_event("authenticated", {"source": "updateAssets"})
        ...
        elif event_type in ("loadHistoryPeriod", "loadHistoryPeriodFast"):
